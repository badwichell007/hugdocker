...
#[test]
fn release_metadata_targets_public_v021_repository() {
    ...
    assert!(readme.contains("DOCKERCTL_VERSION=v0.2.1"));
    ...
    let sha = fs::read_to_string("dockerctl-0.2.1-source.tar.gz.sha256")
        .expect("dockerctl-0.2.1-source.tar.gz.sha256");
    assert!(sha.contains("dockerctl-0.2.1-source.tar.gz"));
}
